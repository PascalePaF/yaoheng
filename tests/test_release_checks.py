...
class ReleaseChecksTests(unittest.TestCase):
    ...
    def test_windows_build_configs_use_expected_release_asset_names(self):
        ...
        self.assertIn('$InstallerCompileRoot = Assert-SafeChildPath', build_script)
        self.assertIn('"/O$InstallerCompileRoot"', build_script)
        self.assertIn('Copy-Item -LiteralPath $CompiledInstallerPath -Destination $InstallerPath -Force', build_script)
        smoke_script = (project_root / "tools" / "smoke_upgrade.ps1").read_text(encoding="utf-8-sig")
        self.assertIn('"YAO_HENG_TEST_INSTANCE_TOKEN"', smoke_script)
        self.assertIn("NewGuid().ToString(\"N\")", smoke_script)
