...
曜衡 3.19 把日常/专业计算、七个法币或虚拟币同步兑换、三端换算、按输入金额查询的 C2C 参考报价以及法币和虚拟币趋势整合在一个桌面应用中，并支持从官方 GitHub Release 检查和覆盖升级。
...
| 版本 | 文件 | 用途 |
| --- | --- | --- |
| Windows 安装版（推荐） | `Yaoheng-3.19-Windows-x64-Setup.exe` | 当前用户安装、覆盖升级、开始菜单、可选桌面快捷方式、标准卸载 |
| 绿色免安装版 | `Yaoheng-3.19-Windows-x64-Portable.zip` | 解压即用，适合 D 盘、移动硬盘或 U 盘 |
...
```powershell
Get-FileHash -Algorithm SHA256 ".\Yaoheng-3.19-Windows-x64-Setup.exe"
Get-FileHash -Algorithm SHA256 ".\Yaoheng-3.19-Windows-x64-Portable.zip"
```
...
| 七币种兑换 | 主币独占首行、六个目标币按三列两行排列；币种搜索与键盘选择、算式输入、同步换算及精确互换 |
...
- 主币数额可直接输入 `+ - * / %` 和括号算式；按回车、输入 `=` 或离开输入框后精确求值，再同步更新六个结果。
- 目标卡可设为主币；“与主币互换”使用内部未舍入的精确结果作为新输入，不拿显示层的短小数反算。
...
- `release\曜衡\曜衡.exe`
- `release\Yaoheng-3.19-Windows-x64-Portable.zip`
- `release\Yaoheng-3.19-Windows-x64-Setup.exe`
