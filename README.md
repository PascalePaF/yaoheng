...
曜衡 3.21.2 把响应式日常/专业计算、七栏 C2C 兑换、独立市场兑换、三端换算、金额匹配报价以及法币和虚拟币趋势整合在一个桌面应用中，并支持持久计算历史、四种界面语言、容错检查官方 GitHub Release 和覆盖升级。
...
| 版本 | 文件 | 用途 |
| --- | --- | --- |
| Windows 安装版（推荐） | `Yaoheng-3.21.2-Windows-x64-Setup.exe` | 当前用户安装、覆盖升级、开始菜单、可选桌面快捷方式、标准卸载 |
| 绿色免安装版 | `Yaoheng-3.21.2-Windows-x64-Portable.zip` | 解压即用，适合 D 盘、移动硬盘或 U 盘 |
...
```powershell
Get-FileHash -Algorithm SHA256 ".\Yaoheng-3.21.2-Windows-x64-Setup.exe"
Get-FileHash -Algorithm SHA256 ".\Yaoheng-3.21.2-Windows-x64-Portable.zip"
```
...
| 模块 | 主要能力 |
| --- | --- |
| 计算器 | 苹果式四列布局与双宽零键；大数字区直接编辑，求值后算式在上、结果在下并常驻两条历史；标准/专业模式随窗口伸缩，历史即时持久保存 |
...
| 虚拟币换算 | 法币与虚拟币三端自由组合，固定使用与市场兑换一致的公开市场参考汇率；C2C 只在独立 C2C 兑换页提供 |
...
设置页的外观区域默认收回，只显示当前主题的五段矩形色块和名称；展开后，每一套主题都单独显示“背景、卡片、按钮、强调色、文字色”预览、配色来源及白/黑文字模式，点击即可即时切换。侧栏“设置”按钮下方提供左右相邻的“上一主题”和“下一主题”，可按画廊顺序双向逐套比较。旧版本保存的主题内部标识保持不变，覆盖升级不会重置选择。
...
C2C 兑换、市场兑换、货币、虚拟币和两个行情页会分别保存自己的币种、主输入、金额、计价币、参考数额、周期和筛选，不会在重启时恢复成默认币种；每次进入这些页面还会自动刷新一次对应行情。计算历史在每次求值或清空时立即写入原子设置文件，即使关闭按钮设为最小化也不会等待最终退出。设置采用 schema v3、原子替换与备份恢复；从旧版本升级时会保留迁移前备份，未来版本设置不会被旧程序覆盖。
...
- `release\曜衡\曜衡.exe`
- `release\Yaoheng-3.21.2-Windows-x64-Portable.zip`
- `release\Yaoheng-3.21.2-Windows-x64-Setup.exe`
