...
| 模块 | 主要能力 |
| --- | --- |
| 计算器 | 苹果式四列布局与双宽零键；首次/求值后新数字替换 `0` 或结果，运算符继续计算；大数字区直接编辑并常驻两条历史 |
...
C2C 兑换、市场兑换、货币、虚拟币和两个行情页会分别保存自己的币种、主输入、金额、计价币、参考数额、周期和筛选，不会在重启时恢复成默认币种；每次进入这些页面还会自动刷新一次对应行情。刷新期间保留现有可信表格和图表，不清空页面或重复播放逐行动画；计算历史在每次求值或清空时立即写入原子设置文件。设置采用 schema v3、原子替换与备份恢复；从旧版本升级时会保留迁移前备份，未来版本设置不会被旧程序覆盖。
...
曜衡使用 Windows 命名互斥保证同时只运行一个窗口。重复点击快捷方式或 EXE 会唤醒、恢复并置前已经运行的窗口。点击标题栏 `×` 会直接保存状态、停止本机 API 并彻底退出；旧版本保存的“关闭时最小化”选项在升级后自动迁移为退出。
